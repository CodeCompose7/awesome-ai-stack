...
- **용어·URL** — `concepts` 컬렉션, `/concepts`·`/concepts/<slug>` 라우트, 한글 라벨 "개념".
- **도구 연결** — 처음부터 **역할 그룹** `tools: [{ role, tools: [slug] }]`. `role`은
  콘텐츠라 로케일 파일별로 직접 작성(ko/en).
- **역링크** — 1차에 도구 상세의 "이 도구가 쓰이는 개념"까지 포함.
- **출시 규모** — *하네스 엔지니어링* 1개 파일럿으로 형식 확정 후 3~4개로 확장.
