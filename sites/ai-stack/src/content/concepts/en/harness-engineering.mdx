...
| --- | --- |
| **Runaway loops** — circling or retrying without end | [[#agent-loop\|A bounded agent loop]] |
| **Risky side effects** — deleted files, arbitrary network calls | [[#code-sandbox\|A code sandbox]] |
| **Unsafe / off-topic output** — leaked data, off-topic detours | [[#guardrails\|Guardrails]] |
| **Silent wrong answers** — confident, plausible falsehoods | [[#evaluation\|Evaluation · tracing]] |
...
How those scores are then thresholded to block or retry is covered under [[#orchestration-the-spine-that-drives-the-cycle|orchestration]] below.
...
The loop calls reason→act→observe in turn — the spine — deciding at each gate whether to stop, retry, or pass, 
and logging every step to *observability* (tracing). 
