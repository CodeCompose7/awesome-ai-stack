...
[[sandboxing]] 개념에서, 모델이 짠 코드는 신뢰 경계 *바깥*이라 일회용·격리 환경에서 돌려야 한다고 했습니다.  
...
- `--network none`으로 네트워크를 끊고, 
- `--memory`·`--cpus`·`--pids-limit`로 메모리·CPU·프로세스 수에 상한을 두고, 
...
*발췌 — 오류 처리는 생략했습니다. 전체 코드는 [구현](#the-implementation) 섹션에 있습니다.*
...
이 접착 계층이 왜 있는지, LangChain 없이 LiteLLM + LangGraph만으로 같은 루프를 직접 구성하면 어떻게 되는지는 [[litellm-langgraph-vs-langchain|별도 비교 글]]에서 다룹니다.
...
도구만 검색으로 바꾸면 [[web-search-fx-agent|오늘의 환율 묻기]], 스크래핑으로 바꾸면 [[web-scraping-agent|문서를 마크다운으로]]가 됩니다.  
같은 에이전트를 LangChain 없이 LiteLLM + LangGraph만으로 구성한 버전은 [[code-sandbox-agent-direct|직접 구성 편]]에 있습니다.  
