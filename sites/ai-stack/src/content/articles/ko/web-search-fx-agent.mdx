...
description: |-
  "오늘 환율?"은 모델 혼자서는 답하지 못합니다.
  Tavily 웹 검색 도구를 하나 붙인 LangGraph 에이전트로, 도구가 학습 컷오프 너머를 어떻게 메우는지 직접 만들어 봅니다.
date: 2026-06-29
version: '1.0'
updated: 2026-06-29
image: '@assets/articles/web-search-fx-agent/main-image.jpeg'
imageAlt: 웹 검색 에이전트 — 오늘의 환율 묻기.
tools: [tavily, langgraph]
related: [web-scraping-agent, code-sandbox-agent, litellm-langgraph-vs-langchain]
project:
  folder: tavily_1
...
[[agent-tools#example-asking-todays-fx-rate]] 개념에서 "오늘의 환율 묻기"를 예로 들었습니다.  
모델의 지식은 학습 시점에 멈춰 있어 환율처럼 매일 바뀌는 값은 알 수 없고, 그 빈틈을 메우는 게 *웹 검색 도구*입니다. 
...
#### `web_search(query)` — 도구 \{#web-search}
...
같은 루프에서 도구만 [[web-scraping-agent|스크래핑(Firecrawl)]]으로 바꾸면 문서를 마크다운으로 읽어 오고, 브라우저 자동화로 바꾸면 또 다른 "지금" 데이터를 끌어올 수 있습니다. 
도구의 갈래는 [[agent-tools]] 개념에 정리해 두었습니다.
