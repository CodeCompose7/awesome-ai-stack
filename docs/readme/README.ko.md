...
| 컬렉션     | 위치                        | 렌더 경로             | 무엇인가                                                          |
| ---------- | --------------------------- | --------------------- | ----------------------------------------------------------------- |
| `stacks`   | `src/content/stacks/`       | `/stack/<slug>/`      | 카탈로그 — 도구/서비스 하나당 항목 하나                            |
| `concepts` | `src/content/concepts/`     | `/concept/<slug>/`    | 여러 도구를 엮는 상위 패턴 (리빙 도큐먼트)                          |
| `articles` | `src/content/articles/`     | `/article/<slug>/`    | 글 — 카탈로그와 분리한 장문 포스트                                  |

세 컬렉션 모두 프런트매터는 [`src/content.config.ts`](../../src/content.config.ts)의
...
```text
src/
├─ content.config.ts            # 세 컬렉션의 Zod 스키마
├─ content/
│  ├─ stacks/{en,ko}/*.mdx      # 로케일별 도구 하나         ← 도구는 여기에 추가
│  ├─ concepts/{en,ko}/*.mdx    # 엮은 패턴                  ← 개념은 여기에 추가
│  └─ articles/{en,ko}/*.mdx    # 글/블로그 포스트           ← 글은 여기에 추가
├─ data/
│  ├─ categories.ts             # 도구 카테고리 트리 (중첩 children, 로케일별 라벨)
│  ├─ concept-categories.ts     # 개념 분류 체계
│  ├─ article-categories.ts     # 글 분류 체계
│  ├─ category-tree.ts          # 공용 트리 헬퍼 (buildTree)
│  └─ glossary.mjs              # [[Term]] 위키링크 용어집 (아래 참고)
├─ i18n/ui.ts                   # 언어, UI 문자열, t() 번역기
├─ lib/
│  ├─ stacks.ts / articles.ts / concepts.ts   # 컬렉션 조회 + 백링크
│  ├─ github.ts                 # 빌드 시점 GitHub 스타/버전 (.aas-cache/ 캐시)
│  ├─ project.ts / samples.ts   # 실행 가능한 샘플 프로젝트 렌더링
│  └─ pricing.ts / dates.ts / inline-md.ts
├─ layouts/BaseLayout.astro     # 공용 셸 (헤더/푸터/테마 + 언어 토글)
├─ components/                  # 약 30개 컴포넌트: 카드, 상세 페이지, 탭,
│                               #   목차 레일, 용어집, 필터, 프로젝트 뷰어…
├─ pages/                       # EN 라우트는 루트에:
│  ├─ index.astro               #   /                홈 (카테고리별 카탈로그)
...
│  └─ ko/…                      #   …그리고 같은 페이지들을 /ko/ 아래에 미러링
└─ styles/global.css            # Tailwind import + Markdown("prose") 스타일
```
...
- **UI 문자열**은 [`src/i18n/ui.ts`](../../src/i18n/ui.ts)의 `ui` 사전에 있습니다.
...
그러면 상세 페이지에 **구현** 탭
([`ProjectViewer`](../../src/components/ProjectViewer.astro))이 뜨고, 프로젝트마다
[`src/lib/project.ts`](../../src/lib/project.ts)가 빌드 시점에:
...
- **Mermaid** 다이어그램은 개요 본문(```mermaid 펜스 블록)과 샘플별 `diagram:`
  ...
  바뀝니다([`MermaidLoader.astro`](../../src/components/MermaidLoader.astro)).
...
[`src/lib/github.ts`](../../src/lib/github.ts)에 있습니다. 레포별로 메모이즈되고
...
있습니다. [`astro.config.mjs`](../../astro.config.mjs)의 `remarkGlossary` 플러그인이
...
- **백링크**(도구 → 글): Astro에는 **네이티브 백링크가 없어서**,
  [`getArticlesForTool()`](../../src/lib/articles.ts)이 빌드 시점에 역방향 조회를
...
스키마([`src/content.config.ts`](../../src/content.config.ts))는 필요할 때 더
