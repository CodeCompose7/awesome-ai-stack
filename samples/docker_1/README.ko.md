...
에이전트 자체를 컨테이너로 띄웁니다 — 호스트 소켓을 마운트해 형제 샌드박스 컨테이너를
띄우는 Docker-out-of-Docker 방식입니다:
...
> 모델이 직접 코드를 짜고 답을 문장으로 정리하므로, 표현이 실행마다 조금씩 달라질 수
> 있습니다. `gemini/gemini-2.5-flash`로 실행한 한 예입니다:
