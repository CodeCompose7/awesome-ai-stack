...
A model's knowledge stops at its training cutoff, and a model can't *do* anything on its own. Tools bridge both gaps — pulling in current data and carrying out real actions.
...
1. **Schema** — each tool's name, description, and input shape are handed to the model, which reads the spec to judge which tool fits.
2. **Filling arguments** — the model picks a tool and generates the input arguments as JSON; a malformed shape is caught by validation and retried.
3. **Using the result** — the call's result feeds back into reasoning to decide the next action — sometimes one call, sometimes several tools chained.
