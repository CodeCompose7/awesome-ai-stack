...
Instead of saying "I don't know," a model hands you a confident, plausible falsehood, and it slips by until a human checks each claim. **Evaluation and tracing** close the gap — rather than trusting a claim, pick the check that fits it: factual claims are scored against trusted sources or a reference, while code and API claims are run in a sandbox or statically checked.
...
For a **factual claim**, the **model** answers and **evaluation** scores it against a **source** (the retrieved context or a reference), filtering out anything below a threshold. In RAG, the very context the model retrieved is what evaluation grades against. **Tracing** records each step — question, model, evaluation — so you can trace back where it went wrong.
