...
**Factual claims** get scored against their source, and anything below a threshold is filtered out.
...
**Code and API claims** are verified by actually running them, like the _code sandbox_ above, or by a static check.

```mermaid
flowchart LR
  call["pandas.read_yaml()"] --> run["run in sandbox · static check"]
  run --> err{"error / missing?"}
