...
A harness isn't one tool but a set of roles, and at its center sits the loop that drives the cycle. It calls reason→act→observe in turn — the spine — and the roles below fill each of those steps.
...
## The roles — what the loop calls at each step

Each role is swappable on its own, and you don't need them all up front — add a layer only where there's a risk to cover. *Model* fills reasoning, *tools & web* and the *code sandbox* fill acting, and guardrails, evaluation, and observability wrap the cycle to validate and measure it.
...
Before output reaches a user, it's checked at runtime, and a violation is blocked or rewritten. For example, `my email is abc@test.com` becomes `my email is [redacted email]`.
...
```mermaid
flowchart LR
  out["output: my email is abc@test.com"] --> guard{"guardrails"}
  guard -- pass --> user["user"]
  guard -- violation --> fix["mask: my email is ⟨redacted⟩"]
  fix --> user
  class guard roleGuard
```
