...
Instead of saying "I don't know," a model hands you a confident, plausible falsehood, and it slips by until a human checks each claim. **Evaluation** catches it; **tracing** records it. Evaluation doesn't take a claim at face value — it picks the check that fits: factual claims are scored against trusted sources or a reference, while code and API claims are run in a sandbox or statically checked. Tracing logs each step so you can trace back where it broke.

<details open>
<summary>Concrete examples</summary>

Silent wrong answers come in several shapes, each caught a different way.

- **Unsourced numbers** — asserting "this job used 20 GPUs" with no basis → scored against the source
- **Subtly wrong facts** — "supported since Python 3.9" when it's really 3.11 → checked against a reference
- **Nonexistent APIs** — calls like `pandas.read_yaml()` that don't exist → run it and it errors; a linter catches it too

For a **factual claim**, the **model** answers and **evaluation** scores it against a **source** (the retrieved context or a reference), filtering out anything below a threshold. In RAG, the very context the model retrieved is what evaluation grades against. Each step — model, evaluation — is logged to **tracing**.
...
**Code and API claims** are verified like the _code sandbox_ above. A **static check** doesn't run the code — it inspects whether the symbols and types exist in the real API, using the installed package or type stubs. A **sandbox** actually runs it in an isolated environment with dependencies in place, catching errors like `AttributeError`, so that execution environment has to be prepared ahead of time.
