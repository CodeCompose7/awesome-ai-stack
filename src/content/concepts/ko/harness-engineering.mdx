...
능력만으로는 신뢰성이 보장되지 않습니다. 아래는 [왜 중요한가](#why-it-matters) 표의 실패 모드를 한 겹씩 막는 층으로, 처음부터 전부 필요하지는 않고 위험이 있는 곳에만 더합니다.
...
파일 삭제나 임의 네트워크 요청 같은 **위험한 부작용**을 격리합니다.  
...
매긴 점수를 임계값으로 걸러 차단·재시도하는 흐름은 아래 [오케스트레이션](#orchestration-the-spine-that-drives-the-cycle)에서 다룹니다.
