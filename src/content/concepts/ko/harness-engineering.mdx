...
각 행은 더 똑똑한 모델이 아니라 모델 바깥의 한 겹으로 해결됩니다 — 각 해결책이 어떻게 동작하는지는 아래 *역할*에서 다룹니다.
...
출력이 사용자에게 가기 전, 런타임에 검사해 위반을 막거나 고칩니다. 예를 들어 `내 이메일은 abc@test.com 이야`는 `내 이메일은 [가려진 이메일] 이야`로 마스킹됩니다.
...
```mermaid
flowchart LR
  out["출력: 내 이메일은 abc@test.com"] --> guard{"가드레일"}
  guard -- 통과 --> user["사용자"]
  guard -- 위반 --> fix["마스킹: 내 이메일은 ⟨가림⟩"]
  fix --> user
