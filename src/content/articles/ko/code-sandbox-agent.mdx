...
이 글에서는 그 *코드 실행* 역할을 실제로 돌아가는 에이전트로 만들어 봅니다 — 도구가 코드를 `exec`하지 않고, 매번 새 Docker 컨테이너에 흘려보냅니다.
...
코드는 호스트에서 절대 돌지 않습니다. `--network none`으로 네트워크를 끊고, 메모리·CPU·프로세스 수에 상한을 두고, `--rm`으로 끝나면 컨테이너를 버립니다.

## 코드 뜯어보기 \{#reading-the-code}

`app.py`의 흐름은 함수 셋으로 나뉩니다 — `main()`이 모델과 에이전트를 짜고, 모델이 부르는 도구가 `run_python()`, 마지막 답을 다듬는 게 `message_text()`입니다.
