...
이 접착 계층이 왜 있는지, LangChain 없이 LiteLLM + LangGraph만으로 같은 루프를 직접 배선하면 어떻게 되는지는 [[litellm-langgraph-vs-langchain|별도 비교 글]]에서 다룹니다.
...
같은 에이전트를 LangChain 없이 LiteLLM + LangGraph만으로 배선한 버전은 [[code-sandbox-agent-direct|직접 배선 편]]에 있습니다.  
