...
`main()`은 이 그래프에 질문을 넣고 마지막 메시지를 출력하는 대여섯 줄이 전부입니다 — `agent.invoke({"messages": […]})` 호출 모양은 원본 글과 똑같습니다. 같은 LangGraph 런타임이니까요.
...
두 버전을 나란히 놓고 무엇을 얻고 잃는지 따져보려면 [[litellm-langgraph-vs-langchain|LangChain 없이 만들면]]을, `create_agent` 두 줄 버전은 [[code-sandbox-agent|원본 튜토리얼]]을 보세요.
