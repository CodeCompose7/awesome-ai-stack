---
title: 코드 샌드박스 에이전트, LangChain 없이 — LiteLLM + LangGraph 직접 배선
description: |-
  같은 코드 샌드박스 에이전트를 이번에는 LangChain 없이 만듭니다. 
  손으로 쓴 도구 스키마와 StateGraph 배선으로, 의존성은 LiteLLM과 LangGraph 둘뿐입니다.
date: 2026-07-03
version: '1.0'
updated: 2026-07-03
...
```mermaid
flowchart TB
  q["작업 (sys.argv)"] --> build["build_graph() — StateGraph 배선"]
...
LiteLLM + LangGraph만으로 배선한 전체 코드입니다.  
...
- **샌드박스는 프레임워크 무관**
  - `run_python`과 격리 플래그는 원조와 동일 — 격리는 배선이 아니라 [샌드박싱](../../concept/sandboxing/) 설계에서 나옵니다.
