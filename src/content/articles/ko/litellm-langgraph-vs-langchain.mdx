---
title: LangChain 없이 만들면 — LiteLLM + LangGraph 직접 배선 vs create_agent
description: |-
  튜토리얼의 에이전트들은 create_agent 한 줄로 루프를 만듭니다. 
  그 한 줄이 감추는 세 계층을 벗겨 보고, LangChain 없이 LiteLLM과 LangGraph만으로 같은 루프를 직접 배선해 봅니다.
date: 2026-07-03
version: '1.0'
updated: 2026-07-03
...
이 글에서는 그 계층을 벗겨 보고, LangChain 없이 같은 루프를 직접 배선하면 무엇이 달라지는지 비교합니다.
...
튜토리얼들이 쓰는 방식입니다. 배선 전체가 이게 끝입니다.
...
같은 루프를 두 라이브러리만으로 직접 배선하면 이렇게 됩니다.  
...
이 방식을 실제로 돌아가는 에이전트로 완성한 튜토리얼이 [[code-sandbox-agent-direct|코드 샌드박스 에이전트 — 직접 배선]]입니다.

## 무엇을 얻고 잃나 \{#tradeoffs}

| | 방법 A — create_agent | 방법 B — 직접 배선 |
| --- | --- | --- |
| 배선 코드 | 두 줄 | 약 50줄 |
...
- **프로토타입·튜토리얼·도구가 주인공인 코드**라면 방법 A입니다. 배선이 두 줄이라 본론(도구 설계, 격리, 프롬프트)에 집중할 수 있고, 이 카탈로그의 튜토리얼들이 전부 이 방식을 쓰는 이유입니다.
- **루프 자체를 배우거나 세밀하게 제어**하려면 — 단계별 로깅, 커스텀 종료 조건, 메시지 가공 — 방법 B가 낫습니다. 의존성 둘을 덜어내는 값으로 배선 코드를 얻는 거래입니다.
- **중간 지점**도 있습니다. [[LangGraph]] 카탈로그 페이지의 두 번째 예제(langgraph_2)는 그래프는 `StateGraph`로 손배선하되 모델·도구는 LangChain 부품(`ChatLiteLLM`·`ToolNode`)을 재사용합니다 — 루프는 투명하게, 접착은 공짜로 가져가는 절충입니다.
