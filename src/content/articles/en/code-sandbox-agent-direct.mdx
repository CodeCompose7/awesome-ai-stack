...
This post rebuilds the **same agent** without LangChain — the only dependencies are [[LiteLLM]] (model routing) and [[LangGraph]] (the loop runtime).  

Why this composition works at all is covered layer by layer in [[litellm-langgraph-vs-langchain|Dropping LangChain]]; here we turn that "option B" into a working agent.
...
Here the schema is your code — the `description` inherits the docstring's job: it's what the model reads to decide *when* to reach for code.
...
The `run_python()` function itself is [identical to the original's](../../article/code-sandbox-agent/#run-python) — it pipes code into a flag-guarded `docker run`, and the sandbox doesn't care which framework called it.
...
`main()` is five or six lines that feed the graph a question and print the last message — and the `agent.invoke({"messages": […]})` call looks exactly like the original's. Same LangGraph runtime, after all.
...
- **The schema is the docs**
  - The `description` field does what `@tool`'s docstring did — you now write the manual the model reads, yourself.
...
To weigh the two versions side by side, see [[litellm-langgraph-vs-langchain|Dropping LangChain]]; for the two-line `create_agent` build, see [[code-sandbox-agent|the original tutorial]].
